export const smci = {
  "name": "Supermicro",
  "ticker": "SMCI",
  "market": "US",
  "roles": [
    "AI Server"
  ],
  "exposure": 38,
  "technicalLevel": "Mid-range",
  "confidence": "medium",
  "summary": "AI server 系統整合節點，適合觀察 BOM 彈性、交期與客戶部署節奏。",
  "customers": [
    "Cloud / enterprise customers"
  ],
  "suppliers": [
    "NVIDIA",
    "Delta Electronics 台達電"
  ],
  "competitors": [
    "Dell",
    "HPE",
    "Quanta 廣達"
  ],
  "alternatives": [
    "Quanta 廣達",
    "Wiwynn 緯穎"
  ],
  "moat": "快速系統設計、整機交付與客戶專案執行。",
  "sources": [
    "supermicroGb200",
    "supermicroLiquid",
    "secEdgar",
    "nasdaqDataLink",
    "occMarketData",
    "cboeOptions"
  ],
  "liveFeeds": {
    "price": {
      "status": "planned",
      "cadence": "delayed or end-of-day until exchange license is configured",
      "sourceKeys": [
        "nasdaqDataLink"
      ]
    },
    "filings": {
      "status": "planned",
      "cadence": "daily plus event-triggered refresh",
      "sourceKeys": [
        "secEdgar"
      ]
    },
    "news": {
      "status": "planned",
      "cadence": "company IR newsroom and exchange disclosure watch queue",
      "sourceKeys": [
        "secEdgar"
      ]
    },
    "options": {
      "status": "planned",
      "cadence": "delayed chain, volume, open interest after data license",
      "sourceKeys": [
        "occMarketData",
        "cboeOptions"
      ]
    },
    "priceSnapshot": {
      "status": "available",
      "last": 33.46,
      "change": 0,
      "changePercent": 0,
      "currency": "USD",
      "asOf": "StockAnalysis public quote snapshot, captured 2026-05-22",
      "provider": "StockAnalysis",
      "sourceKeys": [
        "stockAnalysisQuotes"
      ]
    }
  },
  "industryExposures": {
    "ai-server": {
      "score": 82,
      "label": "AI Server",
      "thesis": "Supermicro 在 AI Server 的曝險主要來自 AI Server 角色、客戶拉貨與供應鏈瓶頸傳導。",
      "drivers": [
        "核心供應鏈節點，需求變化會直接影響訂單與產能配置。",
        "需同時追蹤 SEC filings、產品發布與雲端資本支出。"
      ],
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    },
    "power-cooling": {
      "score": 62,
      "label": "Power & Cooling",
      "thesis": "Supermicro 在 Power & Cooling 的曝險主要來自 AI Server 角色、客戶拉貨與供應鏈瓶頸傳導。",
      "drivers": [
        "屬於關聯供應鏈節點，主要受到客戶資本支出與技術節點切換影響。",
        "需同時追蹤 SEC filings、產品發布與雲端資本支出。"
      ],
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    },
    "optical": {
      "score": 26,
      "label": "Optical Communication",
      "thesis": "Supermicro 在 Optical Communication 的曝險主要來自 AI Server 角色、客戶拉貨與供應鏈瓶頸傳導。",
      "drivers": [
        "屬於關聯供應鏈節點，主要受到客戶資本支出與技術節點切換影響。",
        "需同時追蹤 SEC filings、產品發布與雲端資本支出。"
      ],
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    },
    "memory": {
      "score": 20,
      "label": "Memory",
      "thesis": "Supermicro 在 Memory 的曝險主要來自 AI Server 角色、客戶拉貨與供應鏈瓶頸傳導。",
      "drivers": [
        "屬於關聯供應鏈節點，主要受到客戶資本支出與技術節點切換影響。",
        "需同時追蹤 SEC filings、產品發布與雲端資本支出。"
      ],
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    }
  },
  "roleDetails": [
    {
      "role": "AI Server",
      "detail": "AI server / rack system 交付，受 GPU 配置、電力、液冷、網路與交期瓶頸影響。",
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar"
      ]
    }
  ],
  "capabilityLadder": [
    {
      "level": "High-end / bottleneck-relevant",
      "detail": "AI Server 的高階能力重點在規格密度、量產良率、客戶認證與跨節點整合能力。",
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    },
    {
      "level": "Mainstream / scalable",
      "detail": "主流能力重點在可複製產能、成本控制、交期穩定與多客戶配置。",
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    },
    {
      "level": "Low-end / replaceable",
      "detail": "低階或成熟規格較容易被替代，分析時需和高階規格、客戶黏著度分開看。",
      "sourceKeys": [
        "supermicroGb200",
        "supermicroLiquid",
        "secEdgar",
        "nasdaqDataLink"
      ]
    }
  ],
  "swot": {
    "strengths": [
      {
        "label": "Role clarity",
        "detail": "Supermicro 在 AI Server 供應鏈中角色明確，可從 AI Server 追蹤需求傳導。",
        "sourceKeys": [
          "supermicroGb200",
          "supermicroLiquid",
          "secEdgar",
          "nasdaqDataLink"
        ]
      },
      {
        "label": "Source-backed coverage",
        "detail": "公司檔案連到官方產品、交易所、申報或 IR 來源，適合作為研究入口。",
        "sourceKeys": [
          "supermicroGb200",
          "supermicroLiquid",
          "secEdgar",
          "nasdaqDataLink"
        ]
      }
    ],
    "weaknesses": [
      {
        "label": "Customer / cycle concentration",
        "detail": "曝險高度取決於少數大型客戶、資本支出週期或技術節點切換，需搭配公告與訂單訊號追蹤。",
        "sourceKeys": [
          "supermicroGb200",
          "supermicroLiquid",
          "secEdgar",
          "nasdaqDataLink"
        ]
      }
    ],
    "opportunities": [
      {
        "label": "AI infrastructure pull-through",
        "detail": "AI server、先進封裝、HBM、電力散熱與光通訊的連動可能帶來外溢訂單或規格升級。",
        "sourceKeys": [
          "supermicroGb200",
          "supermicroLiquid",
          "secEdgar",
          "nasdaqDataLink"
        ]
      }
    ],
    "threats": [
      {
        "label": "Qualification and substitution risk",
        "detail": "高階供應鏈需要客戶認證、良率與交期；若規格落後或產能不足，可能被替代供應商分食。",
        "sourceKeys": [
          "supermicroGb200",
          "supermicroLiquid",
          "secEdgar",
          "nasdaqDataLink"
        ]
      }
    ]
  }
};
